const createFlightConfirmationPDF = (doc, order) => {
    ...
    // Flight Details
    const segment = order.serviceDetails.segments[0];
    ...
    // Origin & Destination
    doc.text('Origin:')
       .text(`Airport: ${segment.Origin} (${segment.Origin_City})  ${order.serviceDetails.flightDetails.Origin.COUNTRYCODE}`)
       .text(`Terminal: ${segment.Origin_Terminal}`)
       .moveDown()
       .text('Destination:')
       .text(`Airport: ${segment.Destination} (${segment.Destination_City})  ${order.serviceDetails.flightDetails.Destination.COUNTRYCODE}`)
